...
def parse_filename(filename):
    ...
    # Network is everything between optional YS prefix and the date
    if parts[0].upper() == "YS":
...
def process(csv_path, filename):
    ...
    write_header_row(ws_sum, 2, ["Memo", "Network", "Amount"])
    write_data_cell(ws_sum, 3, 1, memo)
    write_data_cell(ws_sum, 3, 2, deposit_network_full, align=ALIGN_CENTER)
    write_data_cell(ws_sum, 3, 3, receive_payment_amt, fmt="#,##0.00", align=ALIGN_CENTER)
