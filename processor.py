...
def _resolve_shifted_cols(r):
    """If EventDate is not a valid date, an extra column was inserted before Opponent.
    Shift: Venue stays, EventDate col = junk (skip), Section col = real EventDate,
    Row col = Section, Seat col = Row, Qty col = Seat, Reason col = Qty."""
    shifted = not _is_valid_date(r.get("EventDate", ""))
    def _s(val):
...
def build_row(r, remit_date_str, network):
    ...
    return {
        ...
        "Amount": amount,
        "Performer": (r["Opponent"] if not _is_valid_date(r.get("EventDate", "")) and pd.notna(r.get("Opponent")) else (r["Performer"] if pd.notna(r["Performer"]) else "")),
