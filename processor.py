...
def parse_filename(filename):
    ...
    # Date is always the last part, containing hyphens
    date_str = parts[-1]
    d = date_str.split("-")
    year = int(d[2]) if len(d[2]) == 4 else 2000 + int(d[2])
    remit_date = datetime(year, int(d[0]), int(d[1]))

    # Network is everything between the prefix and the date.
    # The prefix is the first part if it does NOT look like a date and
    # is a known prefix (YS, YS2, TV, etc.) — we always skip the first part.
    # If there are only 2 parts (e.g. GoTickets_05-13-2026), the first part IS the network.
    if len(parts) >= 3:
        # Skip first part (prefix like YS, YS2, TV) — network is middle parts
        network_raw = "_".join(parts[1:-1])
    else:
        # Only 2 parts — first part is the network (e.g. GoTickets_05-13-2026)
        network_raw = parts[0]
